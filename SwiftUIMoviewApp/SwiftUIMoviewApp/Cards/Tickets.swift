  ...
  var body: some View {
    ZStack{
      ForEach(tickets) { ticket in
        InfinitStackView(tickets:$tickets , ticket: ticket)
      }
      .padding()
...
struct InfinitStackView : View{
  ...
  @State var ofsset : CGFloat = .zero
  var body : some View {
    VStack {
      Ticket(title: ticket.title, subTitle: ticket.subtitle,top: ticket.top,bottom: ticket.bottom, height: $height)
    }
    .frame(maxWidth: .infinity , maxHeight: .infinity)
    .zIndex(getIndex() == 0 && ofsset > 100 ?        (Double(CGFloat(tickets.count) - getIndex()) - 1) : (Double(CGFloat(tickets.count) - getIndex())))
    ...
    .gesture(
      ...
      .onEnded({ value in
        ...
        let swipeRight = ofsset > (width / 2)
        let swipeLeft = -ofsset > (width / 2)
